...
| TR-181 Parameter | Type | Source API | Description |
|-----------------|------|------------|-------------|
| `Device.Optical.Interface.{i}.TransmitOpticalLevel` | int | `epon_hal_get_transceiver_stats()` | Current optical transmit power level in 0.1dBm units. |
| `Device.Optical.Interface.{i}.OpticalSignalLevel` | int | `epon_hal_get_transceiver_stats()` | Received optical power level in 0.1dBm units. |
| `Device.Optical.Interface.{i}.LowerOpticalThreshold` | int | `epon_hal_get_transceiver_stats()` | Lower receive power threshold in 0.1dBm units. |
| `Device.Optical.Interface.{i}.UpperOpticalThreshold` | int | `epon_hal_get_transceiver_stats()` | Upper receive power threshold in 0.1dBm units. |
| `Device.Optical.Interface.{i}.LowerTransmitPowerThreshold` | int | `epon_hal_get_transceiver_stats()` | Lower transmit power threshold in 0.1dBm units. |
| `Device.Optical.Interface.{i}.UpperTransmitPowerThreshold` | int | `epon_hal_get_transceiver_stats()` | Upper transmit power threshold in 0.1dBm units. |
...
| Parameter | Type | Source API | Description |
|-----------|------|------------|-------------|
| `Device.Optical.Interface.{i}.X_RDK_Transceiver.BiasCurrentMA` | float | `epon_hal_get_transceiver_stats()` | Laser bias current in milliamperes. |
| `Device.Optical.Interface.{i}.X_RDK_Transceiver.TemperatureC` | float | `epon_hal_get_transceiver_stats()` | Module temperature in Celsius. |
| `Device.Optical.Interface.{i}.X_RDK_Transceiver.SupplyVoltageV` | float | `epon_hal_get_transceiver_stats()` | Supply voltage in volts. |
...
1. **Phase 1** (Critical): Core APIs - init, link stats, transceiver stats, manufacturer info, callbacks
2. **Phase 2** (Important): LLID info, interface list, link info, control APIs
3. **Phase 3** (Optional): DPoE support, OAM logging configuration

### 7.2 Telemetry Integration

- Integrate with existing RDK telemetry framework (T2)
- Configure marker upload frequency based on priority
- Implement alert thresholds for critical metrics
- Enable cloud-based monitoring dashboard

### 7.3 Future Enhancements

- Support for 25G-EPON and higher speeds
- Enhanced encryption modes beyond AES-128
- Multi-ONU support for aggregation devices
- Per-LLID statistics for QoS monitoring
- OMCI (ONT Management and Control Interface) integration for GPON/EPON unified management
